...
class AbsoluteMoveNode(Node):
    def __init__(self):
        ...
        self.a_cb_group = ReentrantCallbackGroup()


        # Create an action client for the MoveGroup action
        self.move_group_client = ActionClient(self, MoveGroup, f"/{self.namespace}/move_action", callback_group = self.a_cb_group)
        ...
        self.controller_client = self.make_client(SwitchController, f'/{self.namespace}/controller_manager/switch_controller')
        # self.start_servo_client = self.make_client(Trigger, f'/{self.namespace}/servo_node/start_servo')
        self.planning_scene_client = self.make_client(GetPlanningScene, f'/{self.namespace}/get_planning_scene')

        # Subscribe to current joint states for getting the current robot state
        self.joint_states_sub = self.create_subscription(
            JointState,
            f'/{self.namespace}/platform/joint_states',
            self.joint_states_callback,
            10)

        # self.move_service = self.create_service(RelativeMove, "/relative_move", self.set_callback_flag)
        
        self.move_action = ActionServer(self, AbsoluteMove, 'absolute_move', self.set_callback_flag, callback_group = self.a_cb_group)
        #set robot frames
        ...
        self.get_logger().info('AbsoluteMoveNode is ready.')
    ...
    def set_callback_flag(self, goal_handle):
        ...
        for i in range(2):
            ...
            goal_msg = self.setup_params(goal_handle.request.pose, goal_handle.request.planner, goal_handle.request.pilz_planner) 
    ...
    def get_EE_pose(self, frame_id="base_link"):
    ...
    def create_motion_request(self, planner = "pilz", pilz_planner = "PTP", frame_id = "base_link"):
        #create motion planning request
        motion_request = MotionPlanRequest()
        motion_request.workspace_parameters.header.frame_id = frame_id
        motion_request.workspace_parameters.header.stamp = self.get_clock().now().to_msg()

        # Set the planning group
        motion_request.group_name = "arm_0"
    ...
    def setup_params(self, relative_pose, planner = "ompl",  pilz_planner = "PTP"):
        """Build a moveit Pose request
        """
        # Create a motion planning request
        motion_request = self.create_motion_request(planner, pilz_planner, relative_pose.header.frame_id)
        
        #get current pose
        self.get_EE_pose(relative_pose.header.frame_id)

        #Set target pose
        target_pose = relative_pose
        self.get_logger().info(f"Target Pose: {target_pose}")
        pose = PoseStamped()
        pose.header.frame_id = relative_pose.header.frame_id
        ...
        pose_constraint.header.frame_id = relative_pose.header.frame_id
        ...
        orientation_constraint.header.frame_id = relative_pose.header.frame_id
        ...
        path_constraint.header.frame_id = relative_pose.header.frame_id  # or your world frame
...
def main(args=None):
    ...
    absolute_move_node = AbsoluteMoveNode()

    executor = MultiThreadedExecutor()
    executor.add_node(absolute_move_node)
  
    # rclpy.spin(absolute_move_node)
    try: 
        executor.spin()
    finally:
        absolute_move_node.destroy_node()
        rclpy.shutdown()
